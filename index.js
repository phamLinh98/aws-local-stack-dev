...
// ✅ Mock database cho Secrets Manager (sử dụng .env nếu có)
const SECRET_NAME = process.env.SECRET_NAME;
const secrets = {
  [SECRET_NAME]: JSON.stringify({
    uploadCsvTableName: process.env.UPLOAD_CSV_TABLE_NAME,
    bucketCsvName: process.env.BUCKET_CSV_NAME,
    sqsName: process.env.SQS_NAME,
    prefixQueueURL: process.env.PREFIX_QUEUE_URL,
  }),
};

// ✅ Mock SQS Queues (fallback to .env values)
const queues = [
  process.env.QUEUE_URL_1,
  process.env.QUEUE_URL_2,
];
...
app.listen(PORT, () => {
  console.log(`🔥 Mock S3, DynamoDB, SecretsManager & SQS running at http://localhost:${PORT}`);
});